...
from ...utils.core import vectorize_labels, bbox_overlaps
...
from ...layers import ranking_losses
...
class RankBasedRPNHead(RPNTestMixin, AnchorHead):
    ...
    def loss(self,
        ...
        all_bbox_weights = []
        all_bbox_preds = []
        for labels, label_weights, cls_score, bbox_targets, bbox_weights, bbox_pred in zip(labels_list,
                                                                                           label_weights_list,
                                                                                           cls_scores,
                                                                                           bbox_targets_list,
                                                                                           bbox_weights_list,
        ...
        if pos_idx.sum() > 0:
            ...
            if self.rank_loss_type == 'RankSort':
                ...
                return dict(loss_rpn_rank=self.head_weight * ranking_loss,
                            loss_rpn_sort=self.head_weight * sorting_loss, loss_rpn_bbox=self.head_weight * loss_bbox)
                ...
                losses_bbox = (
                    (torch.cumsum(ordered_losses_bbox, dim=0) / rank[order.detach()].detach().flip(dims=[0])).mean())
                ...
                if self.counter == self.period:
                    self.SB_weight = (np.mean(self.reg_LRP_hist) + np.mean(self.cls_LRP_hist)) / np.mean(
                        self.reg_LRP_hist)
            ...
            if self.rank_loss_type == 'RankSort':
                ...
                return dict(loss_rpn_rank=self.head_weight * ranking_loss,
                            loss_rpn_sort=self.head_weight * sorting_loss, loss_rpn_bbox=self.head_weight * losses_bbox)
            else:
                losses_cls = all_scores.sum() * 0 + 1
                return dict(loss_rpn_cls=self.head_weight * losses_cls, loss_rpn_bbox=self.head_weight * losses_bbox)

    def delta2bbox(self, deltas, means=[0., 0., 0., 0.], stds=[0.1, 0.1, 0.2, 0.2], max_shape=None,
